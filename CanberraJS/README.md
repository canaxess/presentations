...
The bottom two links have an artifically set `TABINDEX` attribute which forces the elements to appear earlier in the keyboard tab sequence than their visual order. Two links with same tabindex value receive keyboard focus according to their DOM order.
